...
from .models import Artwork, ChatSession, Message
...
# 채팅 세션 생성 함수
def create_chat_session(user, artwork):
    return ChatSession.objects.create(user=user, artwork=artwork)
...
# gpt와 대화
def chat_with_gpt(session, user_message):
    # 현재 채팅 세션과 관련된 이전 메세지 가져오기. 맥락에 맞게 대화 이어나가기 위함.
    previous_messages = Message.objects.filter(session=session).order_by('timestamp') # 현재 세션에 해당하는 메세지 가져와 메세지 시간순 정렬.
